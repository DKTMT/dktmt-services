...
class StrategyView(APIView):
    def get(self, request):
        response = requests.request(
                method=request.method, url=predict_service_url + "/strategy", headers=request.headers, data=request.body)
...
class PredictView(APIView):
    ...
    def get(self, request):
        response = requests.request(
                method=request.method, url=predict_service_url + "/predict", headers=request.headers, data=request.body)
        return HTTPResponse(response.content, status=response.status_code)
...
class TradeView(APIView):
    ...
    def get(self, request):
        response = requests.request(
                method=request.method, url=predict_service_url + "/trade", headers=request.headers, data=request.body)
        return HTTPResponse(response.content, status=response.status_code)
